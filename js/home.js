...
(async function () {
  ...
  try {
    ...
    // Find the largest short/medium/long gap. Tiny gaps don't count.
    ...
    if (candidates.length > 0) {
      ...
      // ideaBtn stays hidden — nothing to fill with an idea
    }
  } catch (err) {
  ...
  moodGrid.addEventListener('click', async (e) => {
    ...
    try {
      const recs = await window.getRecommendations(freeMinutes, mood);
  ...
  async function addNearbyOptions(card) {
    try {
      ...
      for (const place of places) {
        ...
        item.textContent = `${place.name} · ${place.type} · ${formatNearbyDistance(place.distanceMeters)}`;
  ...
  function renderRecommendations(recs, mood) {
    ...
    for (const rec of recs) {
      ...
      meta.textContent = `${rec.minutes} min`;
      ...
      function setButtonsDisabled(disabled) {
        for (const { button } of feedbackButtons) button.disabled = disabled;
      }
      ...
      async function submit(button, feedback, rejectionReason = null) {
        ...
        try {
          ...
          if (rec.category === 'GoOut' && (feedback === 'loved' || feedback === 'good')) {
      ...
      function showReasonPicker(noButton) {
        setButtonsDisabled(true);
        actions.innerHTML = '';
        ...
        for (const rejectionReason of reasons) {
          ...
          reasonButton.className = 'btn small';
          reasonButton.textContent = rejectionReason;
